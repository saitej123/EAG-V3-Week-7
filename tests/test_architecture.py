...
def perception_prompt_text() -> str:
    ...
    spec = importlib.util.spec_from_file_location("perception", ROOT / "cognitive_rag" / "perception.py")
    assert spec and spec.loader
    mod = importlib.util.module_from_spec(spec)
    # Extract prompt template by reading source — observe() builds prompt inline.
    src = (ROOT / "cognitive_rag" / "perception.py").read_text(encoding="utf-8")
...
def main() -> int:
    ...
    if (ROOT / "cognitive_rag" / "memory.py").is_file() and "_format_hits" in (
        (ROOT / "cognitive_rag" / "memory.py").read_text()
    ):
