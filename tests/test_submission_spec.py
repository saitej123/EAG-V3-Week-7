...
def test_memory_format_hits():
    assert "_format_hits" in (ROOT / "cognitive_rag" / "memory.py").read_text(encoding="utf-8")
...
def test_mcp_rag_tools_have_corpus_docstrings():
    ...
    from cognitive_rag import mcp_server
...
def test_decision_system_references_mcp_docstrings():
    src = (ROOT / "cognitive_rag" / "decision.py").read_text(encoding="utf-8")
