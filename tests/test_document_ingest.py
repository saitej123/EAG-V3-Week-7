...
from cognitive_rag.documents import (
    INDEXABLE_SUFFIXES,
    DocumentExtract,
    PageExtract,
    citation_label,
...
from cognitive_rag.indexing import _page_source_tag, index_document_path
...
@patch("cognitive_rag.documents._vlm_extract_page", return_value="Page one text")
@patch("cognitive_rag.documents._pdf_page_images", return_value=[(b"png", "image/png")])
@patch("cognitive_rag.documents.normalize_to_pdf", return_value=b"%PDF-stub")
...
def test_index_pdf_with_sidecar_stores_pdf_path(tmp_path, monkeypatch):
    from cognitive_rag import indexing
    ...
    with patch("cognitive_rag.memory.add_fact", side_effect=capture):
...
def test_index_markdown_uses_text_by_default(tmp_path, monkeypatch):
    ...
    with patch("cognitive_rag.memory.add_fact") as mock_add:
...
def test_index_pdf_vlm_path(tmp_path, monkeypatch):
    ...
    with patch("cognitive_rag.indexing.extract_document", return_value=fake):
        with patch("cognitive_rag.memory.add_fact") as mock_add:
