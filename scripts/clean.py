...
Preserves:
  - sandbox/papers/, sandbox/rag_corpus/, sandbox/research_papers/, sandbox/uploads/
  - source code, templates, corpus/MANIFEST.json, pop/
...
KEEP_SANDBOX_DIRS = {"papers", "rag_corpus", "research_papers", "uploads"}
...
def clean_caches() -> None:
    ...
    for sub in ("runs", "tests", "scripts", "tools"):
        base = ROOT / sub
        if not base.is_dir():
            continue
...
def main() -> int:
    ...
    if summary["sandbox"]:
        ...
        print("Sandbox: no extra generated files (papers/ + rag_corpus/ kept).")
    print("Cache: cleared Python/pytest caches outside .venv.")
    print("\nKept: sandbox/papers/, sandbox/rag_corpus/, sandbox/research_papers/, sandbox/uploads/, source, templates, corpus/")
    return 0
