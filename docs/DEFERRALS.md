...
**Where it lives:** `cognitive_rag/memory.py` — vector-first `read()` with keyword overlap as fallback only when embeddings or the index are unavailable.
...
**Where it lives:** `cognitive_rag/indexing.py` — `_chunk_text()`, `index_document_path()`.
...
**Where it lives:** `cognitive_rag/memory.py` — `_load_faiss_from_disk()` on every `read()`.
...
**Where it lives:** `cognitive_rag/llm_env.py` — `gemini_embed_model()`, `try_embed_text()`.
