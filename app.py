...
from cognitive_rag.llm_env import agent_run_max_seconds
...
app = FastAPI(title="Cognitive RAG Agent", lifespan=lifespan)
...
async def run_agent(request: QueryRequest):
    ...
    async def _job():
        try:
            ...
            try:
                from cognitive_rag.agent import _log_final_answer
...
async def index_rag_corpus():
    ...
    try:
        ...
        from cognitive_rag.indexing_async import bulk_index_sidecars_async
...
async def api_documents_catalog():
    ...
    try:
        from cognitive_rag.catalog import get_document_catalog
...
async def api_documents_upload(file: UploadFile = File(...)):
    ...
    if blocked:
        return blocked
    try:
        from cognitive_rag.catalog import save_upload
...
async def api_documents_index(body: DocumentPathRequest):
    ...
    if blocked:
        return blocked
    try:
        from cognitive_rag.indexing_async import index_document_async
...
async def api_documents_reindex(body: DocumentPathRequest):
    ...
    if blocked:
        return blocked
    try:
        from cognitive_rag.catalog import reindex_document
...
async def api_documents_delete_index(path: str):
    ...
    if blocked:
        return blocked
    try:
        from cognitive_rag.catalog import remove_document_index
...
async def api_documents_index_bulk(body: BulkIndexRequest):
    ...
    if blocked:
        return blocked
    try:
        from cognitive_rag.indexing_async import bulk_index_tree_async
        from cognitive_rag.paths import resolve_sandbox_subdir
...
async def api_base_queries():
    ...
    try:
        from cognitive_rag.catalog import base_queries_payload
...
async def api_custom_queries():
    ...
    try:
        from cognitive_rag.catalog import custom_queries_payload
...
async def api_deferrals():
    ...
    from cognitive_rag.catalog import deferrals_payload
...
async def get_prompts_and_evals():
    from tools.validate_pop import POP_EVAL_KEYS, extract_prompt_template
    import json

    def _order_pop_eval(raw: dict) -> dict:
        return {key: raw[key] for key in POP_EVAL_KEYS if key in raw}

    perception_path = BASE_DIR / "cognitive_rag" / "perception.py"
    decision_path = BASE_DIR / "cognitive_rag" / "decision.py"
...
async def run_pop_validation():
    try:
        from tools.validate_pop import async_validate_all_prompts
